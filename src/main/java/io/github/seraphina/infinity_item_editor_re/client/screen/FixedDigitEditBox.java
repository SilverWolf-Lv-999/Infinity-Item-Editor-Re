    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (!value.isEmpty()) {
            ...
            if (!beforeCursor.isEmpty()) {
                guiGraphics.text(this.font, beforeCursor, textX, textY, color, !sidebarUi);
                cursorX = textX + this.font.width(beforeCursor) - 1;
            }

            if (cursorStringPosition < value.length()) {
                guiGraphics.text(this.font, value.substring(cursorStringPosition), cursorX, textY, color, !sidebarUi);
            }
        }

        if (isFocused() && this.cursorFrame / 6 % 2 == 0) {
            guiGraphics.text(this.font, "_", cursorX, textY,
