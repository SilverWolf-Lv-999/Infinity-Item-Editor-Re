...
public class InfinityEditorButton extends AbstractButton {
    ...
    protected void extractContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        renderCenteredModernText(guiGraphics, Minecraft.getInstance().font, x + 2, y, x + width - 2, y + height, textColor);
    }

    private void renderSidebarWidget(GuiGraphicsExtractor guiGraphics) {
        int x = getX();
        int y = getY();
        int width = getWidth();
        int height = getHeight();
    ...
    private void renderCenteredModernText(GuiGraphicsExtractor guiGraphics, Font font, int left, int top, int right, int bottom, int color) {
        int maxWidth = Math.max(0, right - left);
        int centerX = (left + right) / 2;
        int textY = top + (bottom - top - 8) / 2;
        Component message = getMessage();
        if (font.width(message) <= maxWidth) {
            guiGraphics.centeredText(font, message, centerX, textY, color);
            return;
        }
        ...
        guiGraphics.centeredText(font, clipped, centerX, textY, color);
    }
