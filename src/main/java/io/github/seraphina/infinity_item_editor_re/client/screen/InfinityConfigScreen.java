...
public class InfinityConfigScreen extends CompatScreen {
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.centeredText(this.font, this.title, this.width / 2, 12, TEXT_COLOR);
        if (!this.status.getString().isEmpty()) {
            guiGraphics.centeredText(this.font, this.status, this.width / 2, this.height - 38, SAVED_TEXT_COLOR);
        }
    }
    ...
    private static final class ConfigList extends ObjectSelectionList<ConfigEntry> {
        ...
        @Override
        protected void extractSelection(GuiGraphicsExtractor guiGraphics, ConfigEntry entry, int color) {
        }
    }
    private static final class ConfigEntry extends ObjectSelectionList.Entry<ConfigEntry> {
        ...
        public void extractContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovered, float partialTick) {
            ...
            if (this.entry == null) {
                ...
                guiGraphics.text(this.screen.font, modeTitle, left, top + 4, TEXT_COLOR);
                ...
                for (int line = 0; line < Math.min(2, description.size()); line++) {
                    guiGraphics.text(this.screen.font, description.get(line), left, top + 17 + line * 10, MUTED_TEXT_COLOR);
                }
                ...
                guiGraphics.text(this.screen.font, modeText, modeTextX, toggleY + 6, TOGGLE_TEXT_COLOR);
                return;
            }

            boolean value = this.screen.getPendingValue(this.entry);
            guiGraphics.text(this.screen.font, Component.translatable(this.entry.titleKey()), left, top + 4, TEXT_COLOR);
            ...
            guiGraphics.text(this.screen.font, text, textX, toggleY + 6, TOGGLE_TEXT_COLOR);
        }
