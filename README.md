...
| 当前工作区 Fabric API | 0.92.9+1.20.1 |
| Java | Java 17 |
| Mod ID | `infinity_item_editor_re`                           |
| 当前工作区版本 | `Fabric-1.20.1-1.1.0-A`                             |
| 许可证 | GNU GPL 3.0                                         |
...
- 刷怪蛋/刷怪箱：搜索并应用实体类型，支持同步刷怪蛋物品，编辑常用实体标签，并清除 `EntityTag` 或 `SpawnData`。
...
- 通过 Fabric/原版创造标签页 API 提供多个辅助标签页；本版本不带 NeoForge 分支的自定义搜索栏：
...
Fabric 端必须同时安装 Fabric API；当前工作区使用 Fabric API `0.92.9+1.20.1`。
...
配置文件为 `.minecraft/config/infinity_item_editor_re.json`。修改后请重新进入相关界面，必要时重启客户端以确保全部状态刷新。
...
```text
build/libs/infinity_item_editor_re-Fabric-1.20.1-1.1.0-A.jar
```
...
本工作区面向 Fabric 1.20.1。当前版本已经包含放宽后的物品数量、按 Mod 分组的附魔/药水/属性浏览、完整盔甲纹饰编辑器、饰纹陶罐、JSON/命令方块/书本编辑、Fabric 创造标签页和主要快捷键工作流。Minecraft 1.20.1 尚无数据组件和试炼刷怪笼，因此不提供对应入口。高级 NBT 浏览器目前支持结构浏览、展开/折叠和滚动查看，但还不是完整的图形化 NBT 编辑器。
