...
```text
build/libs/infinity_item_editor_re-26.1.2-1.2.0-B.jar
```
...
本工作区面向 Minecraft 26.1.2，并从同一多模块项目构建 Fabric / NeoForge 通用 Jar。当前版本已经包含分类化数据组件编辑、按 Mod 分组的注册表内容、盔甲纹饰、独立收纳袋条目编辑器、饰纹陶罐、试炼刷怪笼、JSON/命令方块/书本编辑、创造标签页和主要快捷键工作流。高级 NBT 浏览器目前支持结构浏览、展开/折叠和滚动查看，但还不是完整的图形化 NBT 编辑器。
