...
- Edit enchantments, potion effects, and registered attributes with mod-namespace grouping, plus armor trims, colors, signs, player heads, armor stands, fireworks, containers, banners/shields, decorated pots, spawn eggs, spawners, trial spawners, villager trades, books, command blocks, and lore.
...
This workspace targets NeoForge `21.1.234` on Minecraft `1.21.1` and builds `build/libs/infinity_item_editor_re-1.21.1-1.5.0-B.jar`.
...
The NeoForge 1.21.1 workspace includes categorized data-component editing, mod-grouped registry lists, armor trims, decorated pots, trial spawners, creative tabs, JSON/command-block/book editing, and the main shortcut workflows. The dedicated bundle-entry editor starts with the 1.21.10 branch. The Advanced NBT Browser remains a read-only structural browser.
...
- 编辑附魔、药水效果和注册表属性，并可按 Mod 命名空间分组；同时支持盔甲纹饰、颜色、告示牌、玩家头颅、盔甲架、烟花、容器、旗帜/盾牌、饰纹陶罐、刷怪蛋、刷怪箱、试炼刷怪笼、村民交易、书本、命令方块和 Lore。
...
当前工作区面向 Minecraft `1.21.1` 和 NeoForge `21.1.234`，构建产物为 `build/libs/infinity_item_editor_re-1.21.1-1.5.0-B.jar`。

这是一个主要面向客户端的工具，服务端通常不需要安装。但写入玩家物品栏、复制实体装备等功能需要创造模式权限，也可能受到服务器规则限制。

### 当前状态

NeoForge 1.21.1 工作区已包含分类化数据组件编辑、按 Mod 分组的注册表列表、盔甲纹饰、饰纹陶罐、试炼刷怪笼、创造标签页、JSON/命令方块/书本编辑和主要快捷键工作流。独立收纳袋条目编辑器从 1.21.10 分支开始提供。高级 NBT 浏览器仍是只读结构浏览器。
