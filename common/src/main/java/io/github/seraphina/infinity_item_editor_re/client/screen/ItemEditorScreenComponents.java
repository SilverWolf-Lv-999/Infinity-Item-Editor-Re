...
abstract class ItemEditorScreenComponents extends ItemEditorScreenActions {
    ...
    protected void renderComponentEditorPanel(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        int left = componentPanelLeft();
        ...
        guiGraphics.text(this.font, Component.translatable(key("components.search")), left, componentFilterY() - 10, labelColor, false);
        guiGraphics.text(this.font, Component.translatable(key("components.available")), left, componentListY() - 10, labelColor, false);
        guiGraphics.text(this.font, componentValueBoxLabel(), valueLeft, componentValueBoxY() - 10, labelColor, false);
        ...
        if (!this.nbtFeedback.isEmpty()) {
            guiGraphics.centeredText(this.font, this.nbtFeedback, this.midX, this.height - 44,
    ...
    private void addSelectedComponentValueControls(int left, int y, int width) {
        ...
        if (isNumberInputComponent(this.selectedComponentKey)) {
            ...
            FilteredEditBox.setFilter(this.componentNumberBox,
                    value -> isAllowedComponentNumberInput(this.selectedComponentKey, value));
    ...
    private void renderComponentList(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, int left, int width) {
        List<ComponentRow> rows = getComponentRows();
        ...
        if (rows.isEmpty()) {
            guiGraphics.text(this.font, Component.translatable(key("components.no_match")),
        ...
        for (int i = this.componentListScroll; i < end; i++) {
            ...
            guiGraphics.text(this.font, clipped, textX, y + 3, textColor, false);
        }

        renderComponentScrollbar(guiGraphics, left, top, width, height, rows.size());
    }

    private void renderComponentValuePreview(GuiGraphicsExtractor guiGraphics, int left, int y, int width) {
        ...
        guiGraphics.text(this.font, clipped, left + 4, y + 6, textColor, false);
    }

    private void renderSelectedComponentValueControls(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, int left, int y, int width) {
        if (!hasSelectedComponent() || !isVanillaComponent(this.selectedComponentKey)) {
            return;
        }

        if (isRegistryValueComponent(this.selectedComponentKey)) {
            guiGraphics.text(this.font, Component.translatable(key("components.control.registry_options")),
        ...
        if (isColorComponent(this.selectedComponentKey)) {
            guiGraphics.text(this.font, Component.translatable(key("components.control.rgb")),
                    left, y, componentLabelColor(), false);
        ...
        if (isNumberInputComponent(this.selectedComponentKey)) {
            guiGraphics.text(this.font, Component.translatable(key("components.control.number")),
                    left, y, componentLabelColor(), false);
            guiGraphics.text(this.font, Component.translatable(key("components.number_hint")),
                    left + Math.min(width, 120) + 8, y + COMPONENT_CONTROL_LABEL_HEIGHT + 6,
                    componentSecondaryTextColor(), false);
        }
    }

    private void renderComponentRegistryChoices(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, int left, int top, int width) {
        List<String> choices = componentRegistryChoices(this.selectedComponentKey);
        ...
        for (int i = 0; i < rows; i++) {
            ...
            guiGraphics.text(this.font, clipped, left + 5, rowY + 3,
                    selected ? componentSelectedTextColor() : componentPrimaryTextColor(), false);
        }

        if (choices.size() > rows) {
            guiGraphics.text(this.font, Component.translatable(key("components.registry_more"), choices.size() - rows),
                    left, bottom + 4, componentSecondaryTextColor(), false);
        }
    }

    private void renderComponentColorPreview(GuiGraphicsExtractor guiGraphics, int left, int y, int width) {
        int color = selectedComponentColorValue();
        ...
        guiGraphics.text(this.font, Component.translatable(key("components.rgb.preview")),
                left + swatchSize + 8, y, componentLabelColor(), false);
        guiGraphics.text(this.font, clipped, left + swatchSize + 8, y + 12, componentSecondaryTextColor(), false);
    }

    private void renderSelectedComponentControls(GuiGraphicsExtractor guiGraphics, int left, int y, int width) {
        List<ComponentControlGroup> groups = selectedComponentGroups();
        if (groups.isEmpty()) {
            guiGraphics.text(this.font, Component.translatable(key("components.control.no_controls")),
        ...
        int groupY = y;
        for (ComponentControlGroup group : groups) {
            guiGraphics.text(this.font, group.label(), left, groupY, componentLabelColor(), false);
            groupY += componentControlGroupHeight(width, group) + COMPONENT_CONTROL_GROUP_GAP;
        }
    }

    private void renderSelectedComponentSummary(GuiGraphicsExtractor guiGraphics, int left, int right, int y) {
        if (!hasSelectedComponent()) {
            return;
        }
        ...
        guiGraphics.text(this.font, componentDisplayName(this.selectedComponentKey), left, y, color, false);
        guiGraphics.text(this.font, this.font.plainSubstrByWidth(this.selectedComponentKey, textWidth), left, y + 14, muted, false);
        guiGraphics.text(this.font, Component.translatable(key("components.status"),
                Component.translatable(key(present ? "components.present" : "components.not_present"))), left, y + 30,
                present ? GOOD_GREEN : muted, false);
        guiGraphics.text(this.font, Component.translatable(key("components.value_type"),
                Component.translatable(key("components.type." + kind.translationKey()))), left, y + 44, muted, false);
        guiGraphics.text(this.font, Component.translatable(key("components.category_label"),
                componentCategoryName(this.selectedComponentKey)), left, y + 58, muted, false);

        int detailY = y + 78;
        if (!defaultValue.isBlank()) {
            guiGraphics.text(this.font, Component.translatable(key("components.default_value")), left, detailY, componentLabelColor(), false);
            guiGraphics.text(this.font, this.font.plainSubstrByWidth(defaultValue, textWidth), left, detailY + 14, muted, false);
            detailY += 34;
        }

        if (isVanillaComponent(this.selectedComponentKey) && !VANILLA_CATEGORY_BY_PATH.containsKey(path)) {
            guiGraphics.text(this.font, Component.translatable(key("components.category.advanced")), left, detailY, muted, false);
        }
    }
    ...
    private String modDisplayName(String namespace) {
        ...
        return PlatformServices.getModDisplayName(namespace)
    ...
    private void renderComponentScrollbar(GuiGraphicsExtractor guiGraphics, int left, int top, int width, int height, int rowCount) {
