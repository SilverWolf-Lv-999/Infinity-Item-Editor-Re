...
This workspace targets NeoForge `21.11.42` on Minecraft `1.21.11` and builds `build/libs/infinity_item_editor_re-1.21.11-1.4.0-B.jar`.
...
The NeoForge 1.21.11 workspace includes categorized data-component editing, mod-grouped registry lists, armor trims, the dedicated bundle-entry editor, decorated pots, trial spawners, creative tabs, JSON/command-block/book editing, and the main shortcut workflows. The Advanced NBT Browser remains a read-only structural browser.
...
当前工作区面向 Minecraft `1.21.11` 和 NeoForge `21.11.42`，构建产物为 `build/libs/infinity_item_editor_re-1.21.11-1.4.0-B.jar`。

这是一个主要面向客户端的工具，服务端通常不需要安装。但写入玩家物品栏、复制实体装备等功能需要创造模式权限，也可能受到服务器规则限制。

### 当前状态

NeoForge 1.21.11 工作区已包含分类化数据组件编辑、按 Mod 分组的注册表列表、盔甲纹饰、独立收纳袋条目编辑器、饰纹陶罐、试炼刷怪笼、创造标签页、JSON/命令方块/书本编辑和主要快捷键工作流。高级 NBT 浏览器仍是只读结构浏览器。
