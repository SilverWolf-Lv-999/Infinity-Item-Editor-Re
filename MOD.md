...
This workspace targets Forge `47.4.20` on Minecraft `1.20.1` and builds `build/libs/infinity_item_editor_re-1.20.1-1.3.0-B.jar`.
...
当前工作区面向 Minecraft `1.20.1` 和 Forge `47.4.20`，构建产物为 `build/libs/infinity_item_editor_re-1.20.1-1.3.0-B.jar`。

这是一个主要面向客户端的工具，服务端通常不需要安装。但写入玩家物品栏、复制实体装备等功能需要创造模式权限，也可能受到服务器规则限制。

### 当前状态

Forge 1.20.1 工作区已包含放宽后的物品数量、按 Mod 分组的注册表列表、完整盔甲纹饰编辑器、饰纹陶罐、创造标签页、JSON/命令方块/书本编辑和主要快捷键工作流。Minecraft 1.20.1 尚无数据组件和试炼刷怪笼，因此不提供对应入口。高级 NBT 浏览器仍是只读结构浏览器。
