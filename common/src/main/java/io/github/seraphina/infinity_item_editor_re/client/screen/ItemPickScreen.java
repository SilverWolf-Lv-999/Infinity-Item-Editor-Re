...
final class ItemPickScreen extends CompatScreen {
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.centeredText(this.font, this.title, this.midX, 15, InfinityEditorButton.MAIN_COLOR);
    ...
    private void renderPickContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        int space = getRenderSpace();
        int currentPage = getCurrentPage();
        int amountPages = getAmountPages();

        guiGraphics.centeredText(this.font, this.pickList.getName(), this.midX, 60, InfinityEditorButton.CONTRAST_COLOR);
        ...
        guiGraphics.centeredText(this.font, searchText, this.midX, 73, BLAND_COLOR);
        ...
        if (currentPage + 1 < amountPages) {
            ...
            guiGraphics.text(this.font, nextPage, this.midX + 10, 70 + TOP_BAR + 168,
                    selected ? InfinityEditorButton.CONTRAST_COLOR : BLAND_COLOR);
        }

        guiGraphics.centeredText(this.font, Integer.toString(currentPage + 1), this.midX, 70 + TOP_BAR + 168, BLAND_COLOR);

        if (currentPage > 0) {
            String previousPage = "<--";
            boolean selected = isMouseIn(mouseX, mouseY, this.midX - nextPageWidth - 10, 70 + TOP_BAR + 168, nextPageWidth, 8);
            guiGraphics.text(this.font, previousPage, this.midX - nextPageWidth - 10, 70 + TOP_BAR + 168,
                    selected ? InfinityEditorButton.CONTRAST_COLOR : BLAND_COLOR);
        }
    ...
    private ItemStack renderPickItems(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, int space, int currentPage) {
        if (this.filteredList.isEmpty()) {
    ...
    private void renderSelectedStack(GuiGraphicsExtractor guiGraphics) {
        ItemStack stack = getSelectedStack();
        if (stack.isEmpty()) {
            return;
        }
        guiGraphics.pose().pushMatrix();
        guiGraphics.item(stack, this.midX - 8, 27);
        guiGraphics.itemDecorations(this.font, stack, this.midX - 8, 27);
        guiGraphics.pose().popMatrix();
    }

    private void renderPickListName(GuiGraphicsExtractor guiGraphics, PickList list, int space, int y) {
        ...
        guiGraphics.text(this.font, name, x, y, color);
    }
