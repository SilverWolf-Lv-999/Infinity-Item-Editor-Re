...
The current 26.1.2 workspace uses Fabric Loader `0.19.3`, Fabric API `0.155.2+26.1.2`, and NeoForge `26.1.2.78`. It builds the universal jar `build/libs/infinity_item_editor_re-26.1.2-1.2.0-B.jar`, which can be installed on Fabric or NeoForge. Fabric API is required on Fabric.
...
The 26.1.2 universal workspace includes categorized data-component editing, mod-grouped registry lists, armor trims, the dedicated bundle-entry editor, decorated pots, trial spawners, both platform creative-tab integrations, JSON/command-block/book editing, and the main shortcut workflows. The Advanced NBT Browser remains a read-only structural browser.
...
当前 26.1.2 工作区使用 Fabric Loader `0.19.3`、Fabric API `0.155.2+26.1.2` 和 NeoForge `26.1.2.78`。通用构建产物为 `build/libs/infinity_item_editor_re-26.1.2-1.2.0-B.jar`，同一个文件可安装到 Fabric 或 NeoForge；Fabric 端必须安装 Fabric API。

这是一个主要面向客户端的工具，服务端通常不需要安装。但写入玩家物品栏、复制实体装备等功能需要创造模式权限，也可能受到服务器规则限制。

### 当前状态

26.1.2 通用工作区已包含分类化数据组件编辑、按 Mod 分组的注册表列表、盔甲纹饰、独立收纳袋条目编辑器、饰纹陶罐、试炼刷怪笼、两种加载器的创造标签页集成、JSON/命令方块/书本编辑和主要快捷键工作流。高级 NBT 浏览器仍是只读结构浏览器。
